...
| Schema evolution | A known older database must upgrade without losing evidence; unknown or outdated schemas must not serve traffic as if ready | E2 `MIG-01` to `MIG-06`, `REG-M01`, and [DEF-008](../defects/DEF-008-unversioned-local-database.md) | `tests/integration/test_database_migrations.py` plus the complete regression gate | In progress |
