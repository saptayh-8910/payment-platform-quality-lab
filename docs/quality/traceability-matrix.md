...
| Asynchronous creation | A delayed request must create one unique reference and deterministic expiry without claiming a financial effect; replay must reuse the original result | E2 `CONF-01`, `CONF-04`, and [catalog](enhancements/e2-asynchronous-payment-confirmation/scenario-catalog.md) | API, database, idempotency, webhook, projection, privacy, migration, and synchronous regression tests | Complete |
| Confirmation authentication | An untrusted sender must not create stored evidence or change financial state | E2 `SEC-C01`, `SEC-C02`, and `PRIV-C01` | Focused HMAC unit tests plus API and database zero-side-effect assertions | Complete |
| Confirmation idempotency | Equivalent confirmation replay must return the original safe result; conflicting identity reuse must stop | E2 `DUP-01` to `DUP-03` | API and integration record, ledger, payment-version, and webhook counts | Complete |
| Confirmation classification | Matching confirmation may capture once; late, mismatched, unknown, or resolved confirmation must remain visible without an unintended financial effect | E2 `CONF-02`, `LATE-01`, `LATE-02`, `MISM-01`, `MISM-02`, `UNK-01`, and `CAP-01` | Atomic rollback, internal diagnostic, ledger, and lifecycle-event tests | Complete |
| Scheduled expiry | An overdue awaiting payment must expire once without a financial effect | E2 `CONF-03` and [scheduled expiry report](enhancements/e2-asynchronous-payment-confirmation/scheduled-expiry-report.md) | Boundary, bounded batch, idempotency, exclusion, rollback, event, and ledger assertions | Complete |
| Receipt and expiry races | On-time accepted work must survive delayed processing; competing triggers must produce one effect | E2 RACE-01 A–N and [race report](enhancements/e2-asynchronous-payment-confirmation/race-resolution-report.md) | Independent SQLite connections, forced checkpoints, recovery, rollback, API retry, and migration tests | Implemented; see report for CI status |
| Awaiting cancellation | Stop an unpaid request without discarding accepted payment evidence | [CANC-01 A–P](enhancements/e2-asynchronous-payment-confirmation/cancellation-review.md) | API, ledger, replay, projection, forced races, failure and retry; [report](enhancements/e2-asynchronous-payment-confirmation/cancellation-report.md) | Implemented and verified; see PR for CI evidence |
| Asynchronous confirmation | A delayed payment must resolve from durable confirmation or explicit expiry without duplicate or ambiguous financial effects | [Enhancement 2 catalog](enhancements/e2-asynchronous-payment-confirmation/scenario-catalog.md) | Creation, confirmation, expiry, race/recovery, cancellation, saved reporting, and EN/JA messaging; [closing evidence](enhancements/e2-asynchronous-payment-confirmation/closing-report.md) | Implemented, verified, and merged through PR #26 |
